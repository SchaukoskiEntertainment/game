...
export default function Jogador({ jogador, eu_id }) {
    ...
    useEffect(() => {
        if (jogador.pantsColor == "green") {
            const color = new Color(86, 145, 86);
            const solver = new Solver(color);
            let colorResult = solver.solve();
            jogador.filtro = colorResult.filterRaw
            setPantsColor("green")
        } else {
            const color = new Color(230, 159, 159);
            const solver = new Solver(color);
            let colorResult = solver.solve();
            jogador.filtro = colorResult.filterRaw
            setPantsColor("red")
    ...
    return (
        <div style={Object.assign(eu_id == jogador.id ? euStyle : outroJogadorStyle)} >
            <div style={{ display: currentAnimation == "idle-right" || currentAnimation == "idle-left" ? "block" : "none" }}>
                ...
                <div style={{ transform: currentAnimation == "idle-right" ? "scaleX(1)" : "scaleX(-1)", position: "absolute", backgroundImage: `url(assets/pants-idle-outline.png)`, backgroundRepeat: "no-repeat", width: 38, height: 55, left: jogador.x, top: jogador.y }}></div>
                <div style={{ filter: jogador.filtro, transform: currentAnimation == "idle-right" ? "scaleX(1)" : "scaleX(-1)", position: "absolute", backgroundImage: `url(assets/pants-idle-color.png)`, backgroundRepeat: "no-repeat", width: 38, height: 55, left: jogador.x, top: jogador.y }}></div>
            </div>
            <div style={{ display: currentAnimation == "walking-right" || currentAnimation == "walking-left" ? "block" : "none" }}>
                ...
                <div style={{ transform: currentAnimation == "walking-right" ? "scaleX(1)" : "scaleX(-1)", animation: "animacaoAvatar 0.8s steps(4) infinite", position: "absolute", backgroundImage: `url(assets/pants-walking-outline.png)`, backgroundRepeat: "no-repeat", width: 38, height: 55, left: jogador.x, top: jogador.y }}></div>
                <div style={{ filter: jogador.filtro, transform: currentAnimation == "walking-right" ? "scaleX(1)" : "scaleX(-1)", animation: "animacaoAvatar 0.8s steps(4) infinite", position: "absolute", backgroundImage: `url(assets/pants-walking-color.png)`, backgroundRepeat: "no-repeat", width: 38, height: 55, left: jogador.x, top: jogador.y }}></div>
            </div>
        </div>
    )
}